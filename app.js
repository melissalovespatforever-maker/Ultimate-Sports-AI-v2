...
async function initApp() {
    ...
    // Check if first visit and not authenticated - show username picker
    setTimeout(() => {
        checkFirstVisitAndPromptUsername();
...
function checkFirstVisitAndPromptUsername() {
    // Don't show if user is authenticated
    if (appState.isAuthenticated && appState.user) {
        console.log('✅ User authenticated, skipping username picker');
        return;
    }
    ...
    if (existingUsername || hasSeenPicker === 'true') {
        console.log('✅ Username already set or picker already shown');
        return;
    }

    // Show the username picker modal
    showGuestUsernamePicker();
}

function showGuestUsernamePicker() {
    console.log('👋 Showing guest username picker');
    ...
    modal.innerHTML = `
        <style>
            ...
            .username-btn-secondary {
                background: rgba(255, 255, 255, 0.2);
                color: white;
                border: 2px solid rgba(255, 255, 255, 0.3);
            }
            .username-btn-secondary:hover {
                background: rgba(255, 255, 255, 0.3);
                border-color: rgba(255, 255, 255, 0.5);
            }
            .username-privacy {
        ...
        <div class="username-modal-content">
            ...
            <h2 class="username-modal-title">Welcome to Ultimate Sports AI!</h2>
            <p class="username-modal-subtitle">
                Choose your avatar and username to personalize your experience
            </p>

            <!-- Avatar Selector -->
            <div class="avatar-selector-section">
                <label class="avatar-selector-label">Choose Your Avatar</label>
                <div class="selected-avatar-display" id="selected-avatar-display">
                    😊
                </div>
                <div class="avatar-grid">
                    ...
                    <div class="avatar-option" data-emoji="🚀" onclick="selectAvatar('🚀')">🚀</div>
                    <div class="avatar-option" data-emoji="🎲" onclick="selectAvatar('🎲')">🎲</div>
                    <div class="avatar-option active" data-emoji="😊" onclick="selectAvatar('😊')">😊</div>
                    <div class="avatar-option" data-emoji="😎" onclick="selectAvatar('😎')">😎</div>
                    <div class="avatar-option" data-emoji="🤩" onclick="selectAvatar('🤩')">🤩</div>
                    <div class="avatar-option" data-emoji="🥳" onclick="selectAvatar('🥳')">🥳</div>
                    <div class="avatar-option" data-emoji="🤑" onclick="selectAvatar('🤑')">🤑</div>
            ...
            <div class="username-input-wrapper">
                <input 
                    ...
                    spellcheck="false"
                >
                <span class="char-counter" id="char-counter">0/20</span>
            </div>

            <div class="username-suggestions">
                <div class="suggestion-chip" onclick="useSuggestion('SportsFan${Math.floor(Math.random() * 9999)}')">🏈 SportsFan</div>
                <div class="suggestion-chip" onclick="useSuggestion('ProGamer${Math.floor(Math.random() * 9999)}')">🎮 ProGamer</div>
                <div class="suggestion-chip" onclick="useSuggestion('ChampPlayer${Math.floor(Math.random() * 9999)}')">🏆 Champion</div>
                <div class="suggestion-chip" onclick="useSuggestion('BetMaster${Math.floor(Math.random() * 9999)}')">💰 BetMaster</div>
            </div>

            <div class="username-buttons">
                <button class="username-btn username-btn-secondary" onclick="skipUsernameSetup()">
                    Skip for Now
                </button>
                <button class="username-btn username-btn-primary" onclick="saveGuestUsername()">
                    Let's Go! 🚀
                </button>
            </div>

            <p class="username-privacy">
                You can change your username anytime in settings.<br>
                Sign up later to save your progress across devices!
            </p>
        </div>
    `;
    ...
    // Initialize selected avatar (default to 😊)
    ...
    setTimeout(() => {
        ...
        if (input) {
            ...
            input.addEventListener('input', (e) => {
                ...
                if (counter) {
                    counter.textContent = `${e.target.value.length}/20`;
                }
            });
...
window.useSuggestion = function(suggestion) {
    ...
    if (input) {
        ...
        const counter = document.getElementById('char-counter');
        if (counter) {
            counter.textContent = `${suggestion.length}/20`;
        }
        input.focus();
    }
};
...
window.saveGuestUsername = function() {
    const input = document.getElementById('guest-username-input');
    let username = input?.value.trim() || '';
    ...
    console.log('✅ Guest username and avatar set:', username, selectedAvatar);

    // Show success toast
    showToast(`Welcome, ${username}! 🎉`, 'success');

    // Close modal with animation
    const modal = document.getElementById('guest-username-modal');
    if (modal) {
...
window.skipUsernameSetup = function() {
    // Mark as seen so we don't show again
    localStorage.setItem('hasSeenUsernamePicker', 'true');

    // Set default guest username and avatar
    const defaultUsername = 'Guest User';
    const defaultAvatar = '😊';
    localStorage.setItem('guestUsername', defaultUsername);
    localStorage.setItem('guestAvatar', defaultAvatar);

    // Update appState
    if (!appState.user) {
        appState.user = {};
    }
    appState.user.name = defaultUsername;
    appState.user.avatar = defaultAvatar;
    appState.user.subscription_tier = 'free';
    appState.notify();

    console.log('✅ Username setup skipped, using default');
...
// Initialize Tier Restrictions if available
if (typeof TierRestrictions !== 'undefined') {
    window.tierRestrictions = new TierRestrictions();
    
    // Set user tier from appState if logged in
    if (appState.user && appState.user.subscription_tier) {
        window.tierRestrictions.setUserTier(appState.user.subscription_tier);
    }
    
    console.log('✅ Tier Restrictions initialized');
}

// Initialize Upgrade Prompts if available
if (typeof initUpgradePrompts === 'function') {
    initUpgradePrompts();
    console.log('✅ Upgrade Prompts initialized');
}
...
window.app = {
    ...
    navigation,
    config: CONFIG,
    tierRestrictions: window.tierRestrictions
};
